...
import { ScanLine, Upload, Check, Loader2, FileImage, Zap, Cpu } from 'lucide-react';
...
export default function Scanner() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (ocrEngine === 'llama') {
      // Use Llama Scout Vision OCR via backend
      try {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">
          आधार कार्ड या अन्य दस्तावेज़ की फोटो अपलोड करें — AI ऑटो-फिल करेगा
        </p>
      </div>
        ...
        {scanning ? (
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-2">JPG, PNG — अधिकतम 5MB</p>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
              <h3 className="font-semibold text-slate-700 flex items-center gap-2">
                ...
                OCR पाठ ({ocrEngine === 'llama' ? 'Llama Scout' : 'Tesseract'})
              </h3>
            </div>
